...
def _get_kernel():
    from biokernel.biokernel.server import BioKernel
    return BioKernel()
...
if HAS_CLICK:
    ...
    def serve(port, host, reload):
        ...
        uvicorn.run(
            "biokernel.biokernel.server:app",
            host=host,
            port=port,
            reload=reload,
        )
    ...
    def run(query, provider, skill):
        ...
        from biokernel.schema.io_types import AgentRequest, ProviderName
    ...
    def eval(eval_file, platform, html):
        ...
        from biokernel.evaluator.eval_engine import EvaluationEngine
    ...
    def mcp():
        ...
        from biokernel.biokernel.mcp_server import MCPServer
        server = MCPServer()
    ...
    def optimize(text, target, output):
        ...
        from biokernel.optimizer.meta_prompter import PromptOptimizer, ModelTarget
    ...
    def catalog():
        ...
        from biokernel.skills_catalog import scan_skills, SKILLS_ROOT
    ...
    def cli():
        ...
        if args.command == "serve":
            import uvicorn
            uvicorn.run("biokernel.biokernel.server:app", host="0.0.0.0", port=args.port)
