...
6.  **dMMR Biomarker Modeling:** Plan colorectal cancer histopathology workflows that explicitly evaluate low-magnification and non-tumor region signals, use slide tiling strategies that preserve tumor and non-tumor region labels, validate MSI/dMMR endpoints, run region-level ablations, and check scanner/cohort bias during external validation.
