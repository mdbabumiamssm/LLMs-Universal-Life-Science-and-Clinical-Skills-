...
13. **Audited multi-region contribution analysis** - Predefine tumor-only, non-tumor-only, low-magnification-only, and combined-region ablations; audit attention or attribution across sampled regions; aggregate region evidence at slide level; confirm performance on external cohorts; and test whether scanner identity, staining batch, fixation, sectioning, or other tissue-processing factors provide shortcut signals before interpreting non-tumor or contextual contributions to dMMR prediction.
