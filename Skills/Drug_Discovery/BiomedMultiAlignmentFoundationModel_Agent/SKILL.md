...
10. Release-specific alignment guardrails: Scope `ibm/biomed.omics.bl.sm.ma-ted-458m` inputs to the repository-supported protein, small-molecule, and single-cell gene modalities, and define protein-small-molecule, protein-single-cell, small-molecule-single-cell, or joint alignment tasks only when the checked-out workflow documents them. Record the exact input schema and returned output type from the relevant example before execution; do not infer that aligned representations imply molecule, protein, or cell-state generation. The source finding names no validation benchmarks or scores, so report only repository-documented benchmark definitions, splits, metrics, and results that were actually reproduced, and label cross-modal retrieval or generation claims as unvalidated unless supported by held-out task-specific evaluation.
11. Licensing and provenance checks: Before using outputs in downstream research, redistribution, or commercial work, inspect the upstream repository and model card for license terms, permitted use, data provenance notes, model version, and access requirements; record the repository URL, commit, model identifier, and any unresolved license or provenance gaps alongside generated embeddings or retrieval results.
