...
> When the instructions reference helper assets such as `skills/dashboard/...` or
> `skills/cjk-viz/...`, map those paths to the Codex tree:
> `repo/Skills/Research_Tools/Reporting/dashboard` and
> `repo/Skills/Data_Visualization/data-visualization/cjk-viz`.
...
3. **Set up Dashboard** — every analysis task must have a live dashboard (templates live in
   `repo/Skills/Research_Tools/Reporting/dashboard/` inside this repository):
   ```bash
   TASK_DIR=data/<task_name>
   mkdir -p "$TASK_DIR/dashboard" "$TASK_DIR/output"
   cp repo/Skills/Research_Tools/Reporting/dashboard/dashboard.html "$TASK_DIR/dashboard/"
   cp repo/Skills/Research_Tools/Reporting/dashboard/dashboard_serve.py "$TASK_DIR/dashboard/"
