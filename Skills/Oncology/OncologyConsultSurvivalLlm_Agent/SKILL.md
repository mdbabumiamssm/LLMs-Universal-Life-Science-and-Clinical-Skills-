...
9. 2026 comparison safeguards: When comparing fine-tuned and zero-shot general LLMs on initial oncology consult notes, enforce temporal cutoffs and leakage controls, evaluate calibration, validate performance across clinically relevant subgroups when data permit, and report outputs cautiously as non-diagnostic estimates requiring clinician review.
10. Survival-aware evaluation: Report discrimination, calibration, and clinically meaningful error analysis for the chosen horizon while avoiding unsupported claims about real-world performance.
11. Clinician-facing reporting: Convert model output into concise risk categories, confidence notes, rationale excerpts, and limitations for review by qualified oncology clinicians.
12. Governance and documentation: Record model version, prompt or training recipe, cohort definition, label code, missingness handling, evaluation date, and intended-use boundaries.
13. Study-aligned comparison framework: Split consultation documents at the patient level so every document from a patient remains in one partition; apply leakage-resistant preprocessing limited to information available at the initial consultation; define censoring-aware endpoints; compare fine-tuned and zero-shot models on identical held-out data using prespecified baseline comparisons, discrimination, and calibration metrics; evaluate clinically relevant subgroups and external cohorts when available; and conclude that fine-tuning offers meaningful clinical benefit only when any gains are reproducible, calibrated, robust across evaluated subgroups and external validation, and relevant to the intended clinical decision rather than limited to isolated metric improvements.
14. Temporal fine-tuning comparison: Use temporally ordered patient-level development and test splits, prevent consultation or outcome information from crossing split boundaries, evaluate censoring-aware endpoints and calibration on the same held-out cohort, seek external validation when available, and explicitly report when fine-tuning does not outperform zero-shot or other simpler prespecified baselines.
