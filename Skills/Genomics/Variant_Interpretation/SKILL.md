...
27. **Evidence-Grounded Genetic Variant Summarization Review**: Retrieve ClinVar, gnomAD, ACMG/AMP evidence, disease database, and literature support before summarization; preserve citation provenance for each retrieved source; separate source assertion strength from speculative interpretation; and report review status for database assertions when available.
28. **Precision-Grounded Clinical Review Summaries**: For genetic variant summaries, retrieve database-backed evidence before synthesis; attribute ClinVar, gnomAD, OMIM-style, or equivalent source records; preserve variant-level provenance for each claim; run hallucination checks for unsupported variant, disease, frequency, inheritance, or pathogenicity statements; and format the result as a structured summary for clinical review.
