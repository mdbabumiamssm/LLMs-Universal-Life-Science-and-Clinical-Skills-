...
4.  **Dermatology/Dermoscopy BCC Evaluation**: For clinical or dermoscopic images involving suspected basal cell carcinoma and common mimickers, apply image-quality gates before analysis (focus, lighting/color, lesion framing, scale, and dermoscopic artifact control) and produce a structured abstention when quality is inadequate. Capture available lesion metadata and clinical context, then return ranked differential diagnoses with calibrated confidence rather than a single label. Compare ChatGPT, Gemini, Claude, or other multimodal models with matched image-plus-clinical prompts and no unsupported performance claims; perform lesion-level error analysis by diagnosis, modality, image quality, and clinically relevant context; report uncertainty, quality limitations, and model disagreements explicitly; and require dermatologist review before any clinical decision, patient-facing guidance, or benchmark interpretation.
