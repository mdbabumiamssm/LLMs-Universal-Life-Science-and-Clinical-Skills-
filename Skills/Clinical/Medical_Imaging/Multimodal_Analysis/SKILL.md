...
4.  **Dermatology/Dermoscopy BCC Evaluation**: For suspected basal cell carcinoma and common mimickers, use paired clinical and dermoscopic images when available and keep both inputs linked at the lesion level. Apply image-quality gates before analysis (focus, lighting/color, lesion framing, scale, and dermoscopic artifact control) and produce a structured abstention when quality is inadequate. Capture available lesion metadata and clinical context, then return ranked differential diagnoses with explicit scores and calibrated confidence rather than a single label. Compare ChatGPT, Gemini, Claude, or other multimodal models with matched image-plus-clinical prompts and no unsupported performance claims; stratify results by image quality, perform lesion-level error analysis by diagnosis, modality, and clinically relevant context, and report sensitivity and specificity when ground-truth labels support them. Report uncertainty, quality limitations, and model disagreements explicitly, and require dermatologist review before any clinical decision, patient-facing guidance, or benchmark interpretation.
