...
4.  **Dermatology/Dermoscopy BCC Caution**: For clinical or dermoscopic images involving suspected basal cell carcinoma and common mimickers, frame outputs as differential considerations only, not a diagnosis. Require image-quality checks before interpretation (focus, lighting, lesion framing, scale/context, and dermoscopic artifact visibility), explicitly list the mimic classes supplied in the clinical prompt or dataset, ask for a differential diagnosis rather than a single label, and require dermatologist confirmation before any clinical decision or patient-facing guidance. Escalate uncertain, discordant, high-risk, or management-changing cases for dermatopathology correlation or biopsy-directed review through appropriate clinical channels. When comparing multimodal LLMs such as ChatGPT, Gemini, and Claude for this task, use the same images, clinical context, prompts, and review rubric across models, and have outputs reviewed by a dermatologist; the finding does not establish standalone diagnostic use.
