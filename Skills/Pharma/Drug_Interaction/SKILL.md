...
37. **Antiseizure Medication DDI Benchmark and Escalation Controls**: Benchmark LLM and Drugs.com antiseizure-medication DDI outputs against Lexicomp or an equivalent authoritative reference; record prompt versions and treat iterative prompting only as a variability check; extract interaction severity and mechanism with source attribution; verify each source's publication or last-updated date; document and preserve disagreements without silently resolving them; and escalate clinically relevant interactions, source conflicts, missing support, uncertain mechanisms, or prompt-dependent conclusions to a pharmacist before clinical use.
38. **Antiseizure Medication DDI Evaluation and Verification**: Evaluate LLM and Drugs.com antiseizure-medication DDI findings against an authoritative reference such as Lexicomp, test sensitivity to iterative prompting, verify interaction severity and mechanism, check for omitted interactions, and state that LLM output cannot replace a validated drug-interaction database.
39. **Antiseizure Medication DDI Concordance Evaluation**: Evaluate LLM and Drugs.com antiseizure-medication DDI outputs against an authoritative reference such as Lexicomp; score severity and mechanism concordance using harmonized categories, run iterative-prompt sensitivity tests, record each source's publication or last-updated date, and require database verification because prompting cannot replace an authoritative DDI database.
40. **Antiseizure Medication DDI Validation Protocol**: Compare LLM outputs with an authoritative compendium such as Lexicomp; assess interaction-severity agreement, omitted interactions, and unsupported claims; record iterative-prompt sensitivity and each source's publication or last-updated date; and require database-grounded verification because prompting cannot replace authoritative DDI verification.
41. **Antiseizure Medication DDI Benchmark Procedure**: Benchmark each antiseizure-medication interaction against an authoritative reference such as Lexicomp, score severity and mechanism agreement using harmonized categories, repeat the assessment with documented iterative prompts to measure answer sensitivity, verify that every interaction claim cites the consulted source, and mandatorily escalate uncertain, uncited, conflicting, or prompt-dependent findings to a pharmacist or qualified clinician before clinical use.
42. **Drug-Interaction Evidence Hierarchy**: Prioritize curated drug-interaction references over LLM recall; compare interaction severity and mechanism across consulted sources, record unresolved conflicts without treating iterative prompting as validation, and require pharmacist review before making consequential recommendations.
43. **LLM Interaction Assessment Benchmark Procedure**: Compare each LLM interaction assessment against authoritative compendia, score severity and mechanism agreement with documented category mappings, repeat the assessment with recorded iterative prompts to test output sensitivity, track interactions omitted by the LLM, verify that cited sources support each assessed interaction, and escalate conflicting source findings to a pharmacist or qualified clinician rather than resolving them through prompting alone.
